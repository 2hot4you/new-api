...
func TestVideoGenerationRoutesIncludeMoliiCompatibilityAliases(t *testing.T) {
	gin.SetMode(gin.TestMode)
	engine := gin.New()
	SetVideoRouter(engine)
	SetTaskPluginProtocolRouter(engine)
	...
	for _, path := range []string{"/v1/videos", "/v1/videos/generations", "/v1/videos/extensions"} {
		_, exists := routes[http.MethodPost+" "+path]
