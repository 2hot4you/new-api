...
func taskPluginProtocolHandlers(protocol, operation string) ([]gin.HandlerFunc, error) {
	...
	case "openai_video.create":
		return []gin.HandlerFunc{
			middleware.RouteTag("relay"), middleware.TokenAuth(), middleware.CaptureVideoStudioRequestSnapshot(), middleware.SystemPerformanceCheck(),
	...
	case "openai_video.content":
		return []gin.HandlerFunc{middleware.RouteTag("relay"), middleware.VideoProxyAuth(), controller.VideoProxy}, nil
	default:
