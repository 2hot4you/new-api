...
func TestPricingCarriesTaskUsageSchemaAndRefreshesWithPluginGeneration(t *testing.T) {
  ...
  seconds: {type: "number", unit: "second", description: "Estimated duration."}
	...
	vendor := Vendor{Name: "Usage Schema Vendor", Description: "Usage schema tests", Icon: "OpenAI", Status: 1}
	require.NoError(t, vendor.Insert())
	for _, modelName := range []string{"pricing-usage-model", "ordinary-model"} {
		row := completePublishedLLM()
		row.ModelName = modelName
		row.DisplayName = modelName
		row.VendorID = vendor.Id
		require.NoError(t, row.Insert())
		configureMarketplaceModelPrice(t, modelName)
	}
...
func TestPricingAliasCarriesPluginUsageSchemaAndTailExpr(t *testing.T) {
  ...
  seconds: {type: "number", unit: "second", description: "Estimated duration."}
	...
	vendor := Vendor{Name: "Alias Usage Schema Vendor", Description: "Alias usage schema tests", Icon: "OpenAI", Status: 1}
	require.NoError(t, vendor.Insert())
	for _, modelName := range []string{"alias-model", "pricing-usage-model", "alias-own-expr"} {
		row := completePublishedLLM()
		row.ModelName = modelName
		row.DisplayName = modelName
		row.VendorID = vendor.Id
