...
function providerLinks(markdown: string) {
...
describe('Provider and model documentation integration', () => {
  test('generated routes are a one-to-one ordered projection of the Development snapshot', async () => {
    ...
    const rootIndex = await source('docs/providers/index.mdx');

    expect(providerLinks(rootIndex)).toEqual(orderedVendors.map((vendor) => ({
      label: vendor.name,
      slug: routeSlug(vendor.name),
    })));
  ...
  test('public documentation routes are unique', async () => {
    ...
    expect(routes.has('/providers')).toBeTrue();
  });
