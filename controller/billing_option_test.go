...
func setupBillingAliasOptionDB(t *testing.T) {
	...
	t.Cleanup(func() {
		// Rebuild the process-wide alias view against a valid, empty database
		// before restoring a possibly nil package-test database handle.
		require.NoError(t, database.Exec("DELETE FROM abilities").Error)
		require.NoError(t, database.Exec("DELETE FROM channels").Error)
