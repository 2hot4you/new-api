...
func updateChannelStarAIBalance(channel *model.Channel) (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to request Molii Volcengine Imagine API balance: %w", err)
	}
	...
	if err := common.Unmarshal(body, &response); err != nil {
		return 0, errors.New("failed to decode Molii Volcengine Imagine API balance response")
	}
	if !response.Code {
		return 0, errors.New("Molii Volcengine Imagine API balance request failed")
	}
	if math.IsNaN(response.Data.BalanceCNY) || math.IsInf(response.Data.BalanceCNY, 0) || response.Data.BalanceCNY < 0 {
		return 0, errors.New("Molii Volcengine Imagine API returned an invalid balance")
	}
	balance := response.Data.BalanceCNY
	if math.IsNaN(balance) || math.IsInf(balance, 0) || balance < 0 {
		return 0, errors.New("converted Molii Volcengine Imagine API balance is invalid")
	}
	channel.UpdateBalance(balance)
	return balance, nil
}
...
func fetchAdvancedCustomBalance(channel *model.Channel) (channelBalanceResult, error) {
	...
	if common.GetJsonType(validated) == "object" {
		...
		if creditSummary.Object == "credit_summary" && common.GetJsonType(creditSummary.TotalAvailable) == "number" {
			var balance float64
			if err := common.Unmarshal(creditSummary.TotalAvailable, &balance); err == nil && balance >= 0 && !math.IsNaN(balance) && !math.IsInf(balance, 0) {
				channel.UpdateBalance(balance)
...
func updateStandardChannelBalance(channel *model.Channel) (float64, error) {
	...
	case constant.ChannelTypeMoliiGrokAIGC:
		return updateChannelMoliiGrokBalance(channel)
	default:
		return 0, errors.New("尚未实现")
	}
