...
func taskBillingSummary(task *model.Task, job *model.TaskBillingJob) *dto.TaskBillingSummary {
	...
	summary := &dto.TaskBillingSummary{State: state, Mode: mode, Model: modelName}
	if state == "settled" {
	...
	if mode == "seedance" {
		...
		summary.DetailAvailable = bc.ActualTokens > 0 && bc.EstimatedUnitPrice >= 0
		return summary
	}
	...
	if snapshot := bc.GrokVideoBilling; snapshot != nil {
		summary.GrokVideo = &dto.TaskGrokVideoBillingV1{
			...
			InputImageCount:          snapshot.InputImageCount,
			...
			VideoInputCost:           snapshot.VideoInputCost,
			Subtotal:                 snapshot.Subtotal,
			GroupRatio:               snapshot.GroupRatio,
			FinalCost:                snapshot.FinalCost,
		}
		summary.DetailAvailable = snapshot.Version == 1 &&
