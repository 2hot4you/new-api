...
全局 Web、全局 API 和已认证搜索限流默认关闭。Session refresh/logout、普通读取接口和日志查询不使用 Critical 限流；登录、注册、密码找回、OAuth 登录等匿名敏感动作继续按 `用途 + 客户端 IP` 使用相互隔离的桶，支付、密钥查看/轮换等已认证敏感动作按 `用途 + 用户 ID` 隔离。Critical 默认额度为 `120 次/10 分钟`，可继续通过 `CRITICAL_RATE_LIMIT`、`CRITICAL_RATE_LIMIT_DURATION` 和 `CRITICAL_RATE_LIMIT_ENABLE` 调整。

Redis 限流使用原子 Lua 固定窗口，替代旧的近似滑动窗口 List 实现。这是有意的语义变化：窗口边界两侧可分别打满一次，极短时间内通过量最高约为配置值的两倍。例如默认 `120 次/10 分钟` 在边界可通过约 240 次。帐户级 Session 上限和签发窗口继续控制数据库增长；如未来需要严格抑制边界突发，需单独迁移为 ZSET 滑动窗口。
...
开放注册仍会受独立的 Critical IP 限流保护，但不会再消耗登录或其他认证动作的额度。分布式 IP 多账号攻击不能仅靠 IP 限流阻止；公网开放注册的部署应同时启用 Turnstile 和邮箱验证，更强的设备或多维风控需作为独立安全项目设计。
