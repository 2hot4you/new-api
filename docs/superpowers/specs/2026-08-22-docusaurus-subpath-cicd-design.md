...
主导航在没有后台 `docs_link` 时已经回退到 `/docs`。部署后管理员应把系统设置中的文档链接设为 `/docs`，使主导航、首页按钮和页脚都使用同域文档；这是运行时配置，不硬编码生产域名。本仓库规则保护既有 New API/QuantumNous 引用，因此本任务不删除或替换上游项目保留的默认文档链接。
