...
### 独立代理实例接入说明（2026-09-25）

本文原有地址是直连 Molii 的示例。接入独立代理实例时，把 Base URL 换成代理商提供的地址，使用代理实例签发给你的本地 Key；不要使用代理商连接 Molii 的实例专属 Key。后台的 `ByteDance Seedance` 代理渠道与 Molii 自用的 `Molii Volcengine Imagine API` 直连渠道是两种配置，部署步骤见 [代理渠道配置指南](ByteDance-Seedance-代理渠道配置指南.md)。公共 API 请求参数不因此改变。

代理实例只启用其上游 Key 获得授权且本地支持的 Seedance 模型；具体可用列表以该实例 `/v1/models` 为准。本文的 2.0 示例不是所有部署的完整模型目录，Mini/2.5 能力另见客户交付指南。零售价格由代理商本地配置，不随模型同步从 Molii 导入。

客户保存、查询和下载时只使用当前实例返回的公共 `task_id`。代理商私有保存 Molii 公共任务 ID；StarAI 原始任务 ID 不属于客户响应，也不能作为查询参数。视频使用本实例鉴权内容接口，支持 GET/HEAD、Range 和 If-Range，不依赖上游签名结果 URL；代理不会重复保存视频。播放暂时失败不会重新生成或重复结算。

临时素材 ID 逐层保持不变，但每层独立检查用户归属。同一用户的有效 Key 可复用其素材，其他用户或其他上游账户不可复用。公网 URL 不需要代理 COS；控制台本地上传需要代理商自身 COS。有效期不超过上游 `expires_at`（通常最多 168 小时）；DELETE 仅清除当前实例绑定及本地 COS 对象，不级联删除上游素材。渠道换成同一 Molii 账户的新 Key 可以继续验证素材，换账户则不可以；历史多 Key 任务的凭据快照限制见配置指南。

计费使用提交时本地价格快照和成功任务的实际 Token，失败按持久化账单退款。成功但用量缺失或不可信时进入 `review_required`，保留预扣等待核查，不表示免费或已完成最终结算。创建 POST 不自动重试；内容/状态 GET 暂时失败时可有界退避。
...
|`reference_image`|多模态参考图|标准/Fast/Mini 为 1–9 张；2.5 为 1–30 张|
...
|单张大小|小于 30 MB|
|数量|首帧 1 张；首尾帧 2 张；标准/Fast/Mini 参考图最多 9 张，2.5 最多 30 张|
